...
def get_oai_dict(filename, numfiles):
	...
	for i in range(1, numfiles+1):
		...
		tree = etree.parse("xmlfile" + filename + str(i) + ".xml")
		root = tree.getroot()
		print root
		...
		for record in records:
			if len(record) and record[0].get("status") != "deleted":
				...
				if "towards tactile user interfaces" in title:
					print title
					for a in authors:
						print a
				#print "the paper is called %s and the authors are:" % title.encode("utf-8")
